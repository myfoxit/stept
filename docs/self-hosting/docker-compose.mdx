...
stept ships with three Docker Compose files:
...
# CORS
CORS_ORIGINS=https://app.yourdomain.com,stept://
...
| `backend` | `ghcr.io/myfoxit/stept-api:latest` | FastAPI backend |
| `media-worker` | `ghcr.io/myfoxit/stept-api:latest` | Celery worker for video processing |
| `frontend` | `ghcr.io/myfoxit/stept-app:latest` | React SPA served via Nginx |
...
This adds:
- **SendCloak** — PII obfuscation proxy (detects and replaces personal data before it reaches AI providers)
- **Presidio** (optional) — Microsoft's NER-based PII detection for enhanced accuracy
...
All services communicate on the `stept-network` bridge network. In production:
