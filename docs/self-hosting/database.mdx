...
stept uses **PostgreSQL 16** with the **pgvector** extension for vector similarity search. The database is managed via SQLAlchemy 2 (async) with Alembic for migrations.
...
```
DATABASE_URL=postgresql+asyncpg://user:password@db:5432/stept
```
...
stept uses [Alembic](https://alembic.sqlalchemy.org/) for database schema migrations.
...
<Note>
If pgvector is not available, stept gracefully falls back to full-text search only. Semantic search will be disabled but all other features work normally.
</Note>
...
stept has 24 tables. Key tables and their purposes:
...
docker compose exec db pg_dump -U postgres stept > backup.sql

# Compressed
docker compose exec db pg_dump -U postgres -Fc stept > backup.dump
```
...
```bash
# From SQL dump
docker compose exec -i db psql -U postgres stept < backup.sql

# From compressed dump
docker compose exec -i db pg_restore -U postgres -d stept < backup.dump
```

## Performance

stept creates indexes on:
...
2. Create the database: `CREATE DATABASE stept;`
