...
stept integrates AI throughout the platform — from auto-processing workflow recordings to inline text editing to a full chat interface. All AI features are optional and require an LLM provider to be configured.
...
| **Custom** | Any OpenAI-compatible endpoint | Set `STEPT_LLM_BASE_URL` |
...
When a workflow recording is finalized, stept automatically runs an AI pipeline:
...
stept supports optional PII obfuscation before data reaches AI providers:
