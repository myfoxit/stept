...
Git Sync exports your stept documents to a Git repository as Markdown files. It's a one-way sync — changes in stept are pushed to Git, but changes in Git are not pulled back.
...
- **Location:** follows the folder hierarchy from stept
...
- Access tokens are **encrypted at rest** using the `STEPT_ENCRYPTION_KEY` Fernet key
...
- **One-way only** — stept → Git. Changes made directly in the Git repo are not synced back.
