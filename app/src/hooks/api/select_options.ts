...
export const useAssignSelectOption = () => {
  ...
  >({
    mutationFn: ({ tableId, rowId, columnId, optionId }) =>
    ...
    onSuccess: (_data, { tableId, columnId }) => {
      qc.invalidateQueries({ queryKey: queryKeys.fields(tableId) }); // rows
      qc.invalidateQueries({ queryKey: queryKeys.selectOptions(columnId) }); // options  ← NEW
    },
  });
};

// Multi-Select Assignment - NEW HOOK
