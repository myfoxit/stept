...
export function DataTable({
  ...
  const [applySorts, setApplySorts] = React.useState(true);
  ...
  const saveVisibilityDebounced = React.useMemo(() => {
    ...
    return (visibility: VisibilityState) => {
      ...
      timeoutId = setTimeout(() => {
        ...
        updateVisibility({ table_id: tableId, visibility: map });
      }, 400);
    };
  ...
  React.useEffect(() => {
    ...
    savedVisibility.forEach((pref) => {
      const c = cols.find((cc) => cc.id === pref.column_id);
  ...
  const handleAddColumn = React.useCallback(
    (name: string, type: string, position?: 'left' | 'right', referenceColumnId?: string) => {
      addColumn({ table_id: tableId, name, type, position, reference_column_id: referenceColumnId });
    },
    [addColumn, tableId]
  );
  ...
  const isLoading = colsLoading || dataWindow.isLoading;
      ...
      <div className="w-full flex flex-col justify-start gap-2">
        ...
        {showImportDialog && (
          <ImportExcelDialog
            ...
            projectId={tableData?.project_id}
