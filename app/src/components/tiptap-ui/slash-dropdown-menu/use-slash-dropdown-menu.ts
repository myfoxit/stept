...
export function useSlashDropdownMenu(config?: SlashMenuConfig) {
  const configRef = React.useRef<SlashMenuConfig | undefined>(config);
  // update the ref on every render
  configRef.current = config;
  const getSlashMenuItems = React.useCallback((editor: Editor) => {
    const items: SuggestionItem[] = [];
    const cfg = configRef.current;

    const enabledItems =
      cfg?.enabledItems || (Object.keys(texts) as SlashMenuItemType[]);
    const showGroups = cfg?.showGroups !== false;
    ...
    enabledItems.forEach((itemType) => {
      ...
      if (itemImpl && itemText && itemImpl.check(editor)) {
        ...
        if (cfg?.itemGroups?.[itemType]) {
          item.group = cfg.itemGroups[itemType];
        } else if (!showGroups) {
          item.group = '';
        }
    ...
    if (cfg?.customItems) {
      items.push(...cfg.customItems);
    }

    console.log('Slash menu items:', config);
