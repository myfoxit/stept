...
interface WorkflowHeaderProps {
  ...
  isEditMode?: boolean;
  onUpdateIcon?: (type: 'tabler' | 'favicon', value: string, color?: string) => void;
...
export function WorkflowHeader({ 
  ...
  const [titleDraft, setTitleDraft] = React.useState<string>(workflow.title ?? 'Untitled workflow');
  ...
  const renderIcon = () => {
    if (iconType === 'favicon' && iconValue) {
      ...
      const IconComponent = (TablerIcons as any)[iconValue] || TablerIcons.IconPencil;
      return <IconComponent className="h-8 w-8 text-white" />;
    }
  };

  const handleIconSelect = (type: 'tabler' | 'favicon', value: string, color?: string) => {
      ...
      <div className="flex items-start gap-4">
        <button
          ...
          className={cn(
            ...
            isEditMode && "cursor-pointer hover:opacity-90 hover:shadow-lg transition-all",
