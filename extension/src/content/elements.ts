...
function _isStableId(id: string): boolean {
  ...
  // React/Radix: contains :r followed by alphanumeric (e.g., :r1ot:, :r3:)
  if (/:r[0-9a-z]/i.test(id)) return false;
  // Radix: _R_ followed by random chars (e.g., _R_5sldpsnpfivbH1_)
  if (/_R_[a-zA-Z0-9]/.test(id)) return false;
  // Starts with : (React useId)
  if (id.startsWith(':')) return false;
  // Common random suffixes: ends with 8+ hex chars or random alphanumeric
  if (/[_-][0-9a-f]{8,}$/i.test(id)) return false;
  // Headless UI / MUI random IDs
  if (/^(headlessui|mui)-/.test(id) && /\d{2,}/.test(id)) return false;
  return true;
}
