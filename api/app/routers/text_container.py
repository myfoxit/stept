...
async def api_list_text_containers(db: AsyncSession = Depends(get_db)):
    return await get_all_text_containers(db)

@router.get("/{container_id}", response_model=TextContainerRead)
async def api_get_text_container(container_id: str,db: AsyncSession = Depends(get_db)):
...
async def api_create_text_container(
    payload: TextContainerCreate, db: AsyncSession = Depends(get_db)
):
...
async def api_update_text_container(
    payload: TextContainerUpdate, db: AsyncSession = Depends(get_db)
):
